...
export class TextClue implements ClueGenerator {
  ...
  private fillTemplate(template: string, city: { name: string; country: string }, rng: () => number): string {
    ...
    const placeholders: Record<string, string> = {
      ...
      '{art}': enhancedCity?.art?.[0] || this.getArtPlaceholder(city.name, rng),
  ...
  private getArtPlaceholder(cityName: string, rng: () => number): string {
    const artTypes = ['famous painting', 'sculpture', 'artwork', 'masterpiece', 'cultural artifact'];
    return artTypes[Math.floor(rng() * artTypes.length)];
  }
